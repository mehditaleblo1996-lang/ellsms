...
The response is a single line of plain text, not JSON — on success, the new `outbound_message.id`; on failure, a negative code: `-1` missing parameter, `-2` authentication failed, `-3` account has no ELLSMS panel access, `-4` invalid destination, `-5` insufficient credit, `-6` the gateway rejected the send.
