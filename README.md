...
The response is JSON: `{"status":"ok","reference_id":123,"error_code":null}` on success (`reference_id` is the new `outbound_message.id`), or `{"status":"not ok","reference_id":null,"error_code":-2}` on failure. `error_code` is one of: `-1` missing parameter, `-2` authentication failed, `-3` account has no ELLSMS panel access, `-4` invalid destination, `-5` insufficient credit, `-6` the gateway rejected the send.
